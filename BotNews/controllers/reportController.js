...
function getReport(req, res) {
  ...
  if (!purchaseService.consumePurchase(report.id, requester)) {
    analytics.recordRequest({ report, agentId: requester, status: 'Payment required' });
    return res.status(402).json(create402Offer({ merchant: 'BotNews', resourceId: report.id, title: report.title, priceInCents: report.priceInCents, currency: 'AUD', paymentProvider: 'Pinch' }));
  }
