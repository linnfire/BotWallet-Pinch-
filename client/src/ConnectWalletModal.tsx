...
export function ConnectWalletModal({ onClose, onConnected }: Props) {
  ...
  return <div className="modal-backdrop" role="presentation"><section className="modal" role="dialog" aria-modal="true" aria-labelledby="connect-title">
    ...
    <form onSubmit={submit}>
      <label>Card Number<input autoComplete="off" inputMode="numeric" required value={card.cardNumber} onChange={(e) => update('cardNumber', e.target.value)} placeholder="1234 5678 9012 3456" /></label>
      <div className="form-row"><label>Expiry Month<input autoComplete="off" inputMode="numeric" required maxLength={2} value={card.expiryMonth} onChange={(e) => update('expiryMonth', e.target.value)} placeholder="01" /></label><label>Expiry Year<input autoComplete="off" inputMode="numeric" required maxLength={4} value={card.expiryYear} onChange={(e) => update('expiryYear', e.target.value)} placeholder="2028" /></label><label>CVC<input autoComplete="off" inputMode="numeric" required maxLength={4} value={card.cvc} onChange={(e) => update('cvc', e.target.value)} placeholder="123" /></label></div>
      <label>Cardholder Name<input autoComplete="off" required value={card.cardHolderName} onChange={(e) => update('cardHolderName', e.target.value)} placeholder="Name on card" /></label>
